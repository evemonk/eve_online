...
describe EveOnline::ESI::CharacterAssets do
  ...
  specify { expect(described_class::API_PATH).to eq("/v5/characters/%<character_id>s/assets/") }
  ...
  describe "#path" do
    specify do
      expect(subject.path).to eq("/v5/characters/12345678/assets/")
    end
  end
  ...
  describe "#url" do
    specify do
      expect(subject.url).to eq("https://esi.evetech.net/v5/characters/12345678/assets/?page=1")
    end
  end
end
