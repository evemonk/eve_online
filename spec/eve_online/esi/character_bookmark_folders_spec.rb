...
describe EveOnline::ESI::CharacterBookmarkFolders do
  ...
  specify { expect(described_class::API_PATH).to eq("/v2/characters/%<character_id>s/bookmarks/folders/") }
  ...
  describe "#path" do
    specify do
      expect(subject.path).to eq("/v2/characters/12345678/bookmarks/folders/")
    end
  end
  ...
  describe "#url" do
    specify do
      expect(subject.url).to eq("https://esi.evetech.net/v2/characters/12345678/bookmarks/folders/?page=1")
    end
  end
end
