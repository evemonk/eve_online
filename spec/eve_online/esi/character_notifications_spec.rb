...
describe EveOnline::ESI::CharacterNotifications do
  ...
  specify { expect(described_class::API_ENDPOINT).to eq('https://esi.evetech.net/v2/characters/%<character_id>s/notifications/?datasource=%<datasource>s') }
  ...
  describe '#url' do
    specify do
      expect(subject.url).to eq('https://esi.evetech.net/v2/characters/12345678/notifications/?datasource=tranquility')
    end
  end
end
