...
describe EveOnline::ESI::CharacterSkillQueue do
  ...
  specify { expect(described_class::API_ENDPOINT).to eq('https://esi.evetech.net/v2/characters/%<character_id>s/skillqueue/?datasource=%<datasource>s') }
  ...
  describe '#url' do
    specify do
      expect(subject.url).to eq('https://esi.evetech.net/v2/characters/12345678/skillqueue/?datasource=tranquility')
    end
  end
end
