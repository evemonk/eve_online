...
describe EveOnline::ESI::UniverseAsteroidBelt do
  ...
  specify { expect(described_class::API_ENDPOINT).to eq('https://esi.evetech.net/v1/universe/asteroid_belts/%<asteroid_belt_id>s/?datasource=%<datasource>s') }
  ...
  describe '#model' do
    ...
    context 'when @model not set' do
      ...
      before do
        #
        # EveOnline::ESI::Models::AsteroidBelt.new(response) # => model
        #
        expect(EveOnline::ESI::Models::AsteroidBelt).to receive(:new).with(response).and_return(model)
      end
