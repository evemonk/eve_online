...
describe EveOnline::ESI::CharacterBookmarks do
  ...
  specify { expect(described_class::API_PATH).to eq("/v2/characters/%<character_id>s/bookmarks/") }
  ...
  describe "#path" do
    specify do
      expect(subject.path).to eq("/v2/characters/12345678/bookmarks/")
    end
  end
  ...
  describe "#url" do
    specify do
      expect(subject.url).to eq("https://esi.evetech.net/v2/characters/12345678/bookmarks/?page=1")
    end
  end
end
